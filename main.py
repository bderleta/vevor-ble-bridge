...
ble_poll_interval=int(os.environ["BLE_POLL_INTERVAL"]) if os.environ.get("BLE_POLL_INTERVAL") else 2
# == Device
device_name=os.environ["DEVICE_NAME"]
device_manufacturer=os.environ["DEVICE_MANUFACTURER"] if os.environ.get("DEVICE_MANUFACTURER") else "Vevor"
...
mqtt_discovery_prefix=os.environ["MQTT_DISCOVERY_PREFIX"] if os.environ.get("MQTT_DISCOVERY_PREFIX") else "homeassistant"
...
def init_client():
    ...
    if (mqtt_username or mqtt_password):
        logger.info("Connecting to MQTT broker %s:%s@%s:%d", mqtt_username, mqtt_password, mqtt_host, mqtt_port)
...
def publish_ha_config():
    ...
    client.publish(mqtt_discovery_prefix + "/button/" + device_id + "-000" + "/config", json.dumps(start_conf))
    ...
    client.publish(mqtt_discovery_prefix + "/button/" + device_id + "-001" + "/config", json.dumps(stop_conf))
    ...
    client.publish(mqtt_discovery_prefix + "/sensor/" + device_id + "-010" + "/config", json.dumps(status_conf))
    ...
    client.publish(mqtt_discovery_prefix + "/sensor/" + device_id + "-011" + "/config", json.dumps(room_temperature_conf))
    ...
    client.publish(mqtt_discovery_prefix + "/sensor/" + device_id + "-012" + "/config", json.dumps(heater_temperature_conf))
    ...
    client.publish(mqtt_discovery_prefix + "/sensor/" + device_id + "-013" + "/config", json.dumps(voltage_conf))
    ...
    client.publish(mqtt_discovery_prefix + "/sensor/" + device_id + "-014" + "/config", json.dumps(altitude_conf))
    ...
    client.publish(mqtt_discovery_prefix + "/number/" + device_id + "-020" + "/config", json.dumps(level_conf))
...
def dispatch_result(result):
    ...
    if (result):
        ...
        if (result.running_step):
            ...
            client.publish(mqtt_prefix+"/heater_temperature/state", result.case_temperature)
